...
from django.contrib import messages
from logging_.models import CheckoutPaymentRequest
...
def stk_push_callback(request): 
        ...
        if resultcode==0:
            ...
            try: 
                messages.success(request,"Payment was successfull please wait to be redirect to oder page",extra_tags='success')
                return HttpResponsePermanentRedirect('/checkout/payment-details/#makeoder')
            except:
                pass
...
def mpesa_payment(request):
    if request.method == "POST":
        ...
        try: 
            ...
            return redirect('/checkout/payment-details/#stk_push')    #stkpush #also sho waiting modal   
        except Exception as error:
            user_agent=request.META['HTTP_USER_AGENT']
            criticality="High"
            error_logging=CheckoutPaymentRequest(phone=phone_number,amount=billed_amount,user=user_agent,critical=criticality,error=error)
            error_logging.save()
            ...
            return redirect('/checkout/payment-details/')     #also sho waiting modal       
